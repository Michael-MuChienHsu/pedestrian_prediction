...
def my_smooth_3d_joints(estimated_3d, max_sliding_window_length, iteration, save_smooth_angle=False):
    '''
    ...
    for e, (name, pair) in enumerate(zip(pair_name, joint_pairs)):
        ...
        smoothed_joint = smooth_angle(anchor, joint,max_sliding_window_length, "%s_%d"%(name, iteration), save_smooth_angle=False) 
        smoothed_3d_joints[:,pair[1],:] = smoothed_joint

    return smoothed_3d_joints

def smooth_angle(anchor, joint, max_sliding_window_length, pair_name, save_smooth_angle):
    '''
    anchor: N x 3
    joint: N x 3
    '''
